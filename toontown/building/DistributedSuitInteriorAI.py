...
from toontown.battle import DistributedBattleBldgAI, DistributedBattleBoilerAI
from toontown.battle import BattleBase
from direct.task import Timer
import DistributedBoilerAI
...
class DistributedSuitInteriorAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def enterElevator(self):
        if self.currentFloor == self.topFloor:
            suitHandles = self.bldg.planner.genFloorSuits2(self.currentFloor)
        else:
            suitHandles = self.bldg.planner.genFloorSuits(self.currentFloor)
        self.suits = suitHandles['activeSuits']
        self.activeSuits = []
        if self.currentFloor == self.topFloor:
            self.boiler = DistributedBoilerAI.DistributedBoilerAI(self.air)
            for toonId in self.toonIds:
                self.boiler.d_appendToonIds(toonId)
            self.boiler.generateWithRequired(self.zoneId)
    ...
    def __createFloorBattle(self):
        ...
        if self.currentFloor == self.topFloor:
            self.battle = DistributedBattleBoilerAI.DistributedBattleBoilerAI(self.air, self.zoneId, self.__handleRoundDone, self.__handleBattleDone, bossBattle=bossBattle)
            self.battle.suitRequestJoin(self.boiler)
        else:
    ...
    def __handleRoundDone(self, toonIds, totalHp, deadSuits):
        ...
        if self.currentFloor == self.topFloor:
            ...
            if len(self.reserveSuits) > 0 and len(self.activeSuits) < 5:
                for info in self.reserveSuits:
                    if len(self.activeSuits) < 5:
