...
def getLeftClosePointMid(type):
    width = ElevatorData[type]['width']
    return Point3(1.3, 0, 0)


def getRightClosePointMid(type):
    width = ElevatorData[type]['width']
    return Point3(-1.3, 0, 0)
...
def getLeftOpenInterval(distObj, leftDoor, type):
    ...
    midPos = getLeftClosePointMid(type)
    leftOpenInterval = LerpPosInterval(leftDoor, openTime, openPos, startPos=closedPos, blendType='easeIn', name=distObj.uniqueName('leftDoorOpen'))
    leftOpenInterval2 = LerpPosInterval(leftDoor, openTime, midPos, startPos=openPos, blendType='noBlend', name=distObj.uniqueName('leftDoorOpenMid'))
    leftOpenInterval3 = LerpPosInterval(leftDoor, openTime, openPos, startPos=midPos, blendType='easeOut', name=distObj.uniqueName('leftDoorOpen'))
    openInterval = Sequence(leftOpenInterval, leftOpenInterval2, leftOpenInterval3)
    return openInterval
...
def getRightOpenInterval(distObj, rightDoor, type):
    ...
    midPos = getRightClosePointMid(type)
    rightOpenInterval = LerpPosInterval(rightDoor, openTime, openPos, startPos=closedPos, blendType='easeIn', name=distObj.uniqueName('rightDoorOpen'))
    rightOpenInterval2 = LerpPosInterval(rightDoor, openTime, midPos, startPos=openPos, blendType='noBlend', name=distObj.uniqueName('rightDoorOpenMid'))
    rightOpenInterval3 = LerpPosInterval(rightDoor, openTime, openPos, startPos=midPos, blendType='easeOut', name=distObj.uniqueName('rightDoorOpen'))
    openInterval = Sequence(rightOpenInterval, rightOpenInterval2, rightOpenInterval3)
    return openInterval
