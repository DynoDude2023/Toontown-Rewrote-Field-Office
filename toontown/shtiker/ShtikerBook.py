...
class ShtikerBook(DirectFrame, StateData.StateData):
    ...
    def addPageTab(self, page, pageIndex, pageName = 'Page'):
        ...
        if pageName == TTLocalizer.OptionsPageTitle:
            ...
            iconModels.detachNode()
        elif pageName == 'Resistance Rank':
            iconModels = loader.loadModel('phase_3.5/models/gui/ttr_m_gui_sbk_resistanceRank_icons')
            iconGeom = iconModels.find('**/resistanceRank/stamp_grp/ttr_t_gui_sbk_resistanceRank_stamp_0_trainee')
            iconModels.detachNode()
        if pageName == TTLocalizer.OptionsPageTitle:
