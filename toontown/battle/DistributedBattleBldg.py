...
import DistributedBattleFieldOffice
...
class DistributedBattleBldg(DistributedBattleFieldOffice.DistributedBattleFieldOffice):
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedBattleBldg')
    camFOFov = 30.0
    camFOPos = Point3(0, -10, 4)

    def __init__(self, cr, isBoiler=False):
        townBattle = cr.playGame.getPlace().townBattle
        DistributedBattleFieldOffice.DistributedBattleFieldOffice.__init__(self, cr, townBattle, isBoiler=isBoiler)
        self.streetBattle = 0
    ...
    def generate(self):
        DistributedBattleFieldOffice.DistributedBattleFieldOffice.generate(self)
    ...
    def disable(self):
        DistributedBattleFieldOffice.DistributedBattleFieldOffice.disable(self)
        self.battleMusic.stop()

    def delete(self):
        DistributedBattleFieldOffice.DistributedBattleFieldOffice.delete(self)
                ...
                for do in self.cr.doId2do.values():
                    if isinstance(do, DistributedBoiler):
                        if base.localAvatar.doId in do.getToonIds():
                            ...
                            actorInterval = ActorInterval(self.boiler, 'intro')
                            oneSuitTrack.append(actorInterval)
                            oneSuitTrack.append(Func(self.boiler.loop, 'idle'))
                            break
        else:
            for suit in self.suits:
                suit.setState('Battle')
