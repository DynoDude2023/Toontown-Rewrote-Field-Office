...
def doSuitAttack(attack):
    ...
    elif name == OFFENSE_MODE:
        suitTrack = doOffenseMode(attack)
    elif name == BURN_OUT:
        suitTrack = doBurnOut(attack)
    elif name == PROMOTION:
    ...
    for do in base.cr.doId2do.values():
        if isinstance(do, DistributedBoiler):
            if base.localAvatar.doId in do.getToonIds():
                boiler = do
                if boiler.getMode() == 'offense':
                    neutralAnim = 'offenseIdle'
                else:
                    neutralAnim = 'idle'
                seq = Sequence()
                soundTrack = getSoundTrack('ttr_s_chr_cbg_boss_offenseAttack.ogg', delay=0.3, node=suit)
                actorInterval = ActorInterval(boiler, 'offenseAttack')
                toonTracks = getToonTracks(attack, 1.8, ['slip-forward'], 1.29, ['jump'])
                par = Parallel(actorInterval, soundTrack, Sequence(Wait(1.3), toonTracks))
                parSeq = Sequence(par, Func(boiler.loop, neutralAnim))
                seq.append(parSeq)
...
def getGearLift(platformModel):
    return platformModel.find('**/geo_gearLift_01')
    
def doShow(platformModel, suit, pos, hpr):
    platformModel.reparentTo(render)
    platformModel.setPos(pos)
    platformModel.setHpr(hpr)
    platformModel.setScale(1.0)
    
def doHide(platformModel):
    platformModel.hide()
    
def doPopup(platformModel, suit, pos, platFormDelay):
    from direct.showbase.MessengerGlobal import messenger
        
    platform = getGearLift(platformModel)
        
    platform.show()
    platform.setScale(0.001)
        
    platFormScaleIntervalIn = LerpScaleInterval(platform, platFormDelay + 0.2, 1.0, blendType='easeInOut')
    platFormScaleIntervalOut = LerpScaleInterval(platform, platFormDelay + 0.2, 0.001, blendType='easeInOut')
        
    platFormPlatUp = LerpPosInterval(platform, 1.5, (pos[0], pos[1], pos[2] + 2.5), blendType='easeInOut')
    platFormCogUp = LerpPosInterval(suit, 1.5, (pos[0], pos[1], pos[2] + 2.5), blendType='easeInOut')
        
    platFormPlatDown = LerpPosInterval(platform, 1.0, (pos[0], pos[1], pos[2]), blendType='easeInOut')
    platFormCogDown = LerpPosInterval(suit, 1.0, (pos[0], pos[1], pos[2]), blendType='easeInOut')
        
    platformShow = Sequence(Wait(platFormDelay), platFormScaleIntervalIn, Wait(0.5),
                            Parallel(platFormPlatUp, platFormCogUp, Func(messenger.send, 'level-up-cogs')), Wait(1.0),
                            Parallel(platFormPlatDown, platFormCogDown), Wait(1.0), Func(doHide, platformModel)).start()
        ...
        if isinstance(do, DistributedBoiler):
            if base.localAvatar.doId in do.getToonIds():
                ...
                for targetSuit in battle.activeSuits:
                    if targetSuit != suit:
                        targetedCogs.append(targetSuit)
                
                suitTracks = Parallel()
                extraDelay = 0.0
                for targetSuit in targetedCogs:
                    platformModel = loader.loadModel('phase_5/models/props/ttr_m_ara_cbg_promoted')
                    extraDelay += 0.2
                    suitTracks.append(Func(doShow, platformModel, targetSuit, targetSuit.getPos(), targetSuit.getHpr()))
                    suitTracks.append(Wait(0.5))
                    suitTracks.append(Func(doPopup, platformModel, targetSuit, targetSuit.getPos(), 0.8+extraDelay))

                par = Parallel(actorInterval, soundTrack, Sequence(Wait(1.0), suitTracks))
                parSeq = Sequence(par, Func(boiler.loop, neutralAnim))
                seq.append(parSeq)
                
                return seq

def doOffenseMode(attack):
    suit = attack['suit']
    from toontown.building.DistributedBoiler import DistributedBoiler
    for do in base.cr.doId2do.values():
        if isinstance(do, DistributedBoiler):
            if base.localAvatar.doId in do.getToonIds():
                boiler = do
                boiler.setMode('offense')
                seq = Sequence()
                LerpColorScaleInterval(render, 3, Vec4(1, 0.75, 0.75, 0.75))
                soundTrack = getSoundTrack('ttr_s_chr_cbg_boss_defenseIntoOffense.ogg', delay=0.3, node=suit)
                actorInterval = ActorInterval(boiler, 'defenseIntoOffense')
                par = Parallel(actorInterval, soundTrack)
                parSeq = Sequence(par, Func(boiler.loop, 'offenseIdle'))
                seq.append(parSeq)
