...
def chooseSuitShot(attack, attackDuration):
    ...
    if name == AUDIT:
        ...
        camTrack.append(allGroupHighShot(suit, 6.0))
    elif name == OFFENSE_MODE or name == PROMOTION:
        camTrack.append(allGroupHighShot(suit, 6.0))
    elif name == POUND_KEY:
        camTrack.append(defaultCamera(openShotDuration=2.8))
    elif name == POWER_TIE:
...
def allGroupLowShot(avatar, duration):
    return heldShot(43, -17, 9.94, 64, 0, 0, duration, 'allGroupLowShot')
...
def allGroupHighShot(avatar, duration):
    return heldShot(14, -5, 3, 82, 12, 0, duration, 'allGroupLowShot')
