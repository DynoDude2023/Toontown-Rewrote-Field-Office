...
class BattleBaseBoiler:
    ...
    def calcSuitMoveTime(self, pos0, pos1):
        ...
        return dist / BattleBaseBoiler.suitSpeed

    def calcToonMoveTime(self, pos0, pos1):
        dist = Vec3(pos0 - pos1).length()
        return dist / BattleBaseBoiler.toonSpeed

    def buildJoinPointList(self, avPos, destPos, toon=0):
        minDist = 999999.0
        nearestP = None
        for p in BattleBaseBoiler.allPoints:
        ...
        if dist < minDist:
            ...
            return []
        if toon == 1:
            if nearestP == BattleBaseBoiler.posE:
                self.notify.debug('buildJoinPointList() - posE')
                plist = [BattleBaseBoiler.posE]
            elif BattleBaseBoiler.toonCwise.count(nearestP) == 1:
                self.notify.debug('buildJoinPointList() - clockwise')
                index = BattleBaseBoiler.toonCwise.index(nearestP)
                plist = BattleBaseBoiler.toonCwise[index + 1:]
            else:
                self.notify.debug('buildJoinPointList() - counter-clockwise')
                index = BattleBaseBoiler.toonCCwise.index(nearestP)
                plist = BattleBaseBoiler.toonCCwise[index + 1:]
        elif nearestP == BattleBaseBoiler.posA:
            self.notify.debug('buildJoinPointList() - posA')
            plist = [BattleBaseBoiler.posA]
        elif BattleBaseBoiler.suitCwise.count(nearestP) == 1:
            self.notify.debug('buildJoinPointList() - clockwise')
            index = BattleBaseBoiler.suitCwise.index(nearestP)
            plist = BattleBaseBoiler.suitCwise[index + 1:]
        else:
            self.notify.debug('buildJoinPointList() - counter-clockwise')
            index = BattleBaseBoiler.suitCCwise.index(nearestP)
            plist = BattleBaseBoiler.suitCCwise[index + 1:]
