...
class DistributedBossCogAI(DistributedAvatarAI.DistributedAvatarAI):
    ...
    def generatePlannerSuits(self, cogAmt, minLevel, maxLevel):
        diners = []
        for i in xrange(cogAmt):
            suitType = random.randint(1, 8)
            suitLevel = random.randint(minLevel, maxLevel)
            if suitLevel > suitType + 4:
                suitLevel = suitType + 4
