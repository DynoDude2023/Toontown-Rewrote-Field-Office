...
class DistributedNPCSpecialQuestGiver(DistributedNPCToonBase):
    ...
    def announceGenerate(self):
        ...
        npcOrigin = self.cr.playGame.hood.loader.geom.find('**/npc_questgiver_origin' + `(self.posIndex)`)
        if not npcOrigin.isEmpty():
            self.reparentTo(npcOrigin)
            self.clearMat()
        else:
            self.notify.warning('announceGenerate: Could not find npc_questgiver_origin' + str(self.posIndex))
